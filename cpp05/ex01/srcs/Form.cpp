...
{
	if (inst.isSigned())
		stream << "Form " << inst.getName() << " is true, the grade for signedStatus is :" << inst.getRankExec();
	else
		stream << "Form " << inst.getName() << " is false, the grade for signedStatus is :" << inst.getRankExec();
	return (stream);
}